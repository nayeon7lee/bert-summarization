...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # input_batch = list of 1-hot vector representation
    ...
    def create_ptr_and_gate(self, input_batch, target_batch, in_tex,out_tex):
        ptr = []
        gate = []
        for tok in target_batch:
            index = [loc for loc, val in enumerate(list(input_batch)) if (val == tok)]
            ptr_temp = [0 for _ in range(len(list(input_batch)))]
            if(index): 
                gate.append(1)
                ## create dist
                for id_ptr in index:
                ...
                ## select sentinel if nothing to copy
                gate.append(0)
                ptr_temp[0] = 1
            ptr.append(ptr_temp)
    
        return torch.FloatTensor(ptr), torch.FloatTensor(gate)
...
def prepare_data_seq():
    ...
    vocab = Lang({config.UNK_idx: "UNK", config.PAD_idx: "PAD", config.EOS_idx: "EOS", config.SOS_idx: "SOS", config.SENTINEL: "SENT"})
...
class Personas:
    ...
    def get_balanced_loader(self,persona,batch_size,split, fold=-1):
